    ...
    <div class="serve-bg">
      <div class="serve-table">
        <table border="0" cellspacing="0" cellpadding="0" class="tableStyle">
          ...
          <tbody class="serveList" style="text-align: center" v-if="serveList.length> 0">
            <tr
              v-for="(item, index) in serveList"
              :key="index"
              class="bableBody"
            >
              <td style="width: 200px; text-align:left; padding-left:52px;">{{ item.waiterName }}</td>
              <td>{{ item.orderNum }}</td>
              <td>{{ item.completeOrderNum }}</td>
              <td>{{ item.serviceLength }}</td>
              <td>{{ item.visitNum }}</td>
              <td>{{ item.commentNum }}</td>
              <td>{{ item.goodCommentNum }}</td>
              <td>{{ item.goodCommentRate }}%</td>
              <td>{{ item.badCommentRate }}%</td>
              <td>{{ item.trainNum }}%</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="block" style="text-align: center; margin: 34px 0 36px 0" v-if="serveList.length> 0">
        <el-pagination
          @size-change="handleSizeChange"
          @current-change="handleCurrentChange"
          :current-page.sync="listServePerson.pageNum"
          :page-size="listServePerson.pageSize"
       ...
       <div class="default" v-if="serveList.length == 0">
       <img
          class="default-img"
...
export default {
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      this.listServePerson.pageSize = val;
      // this.getServePerson();
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.listServePerson.pageNum = val;
      // this.getServePerson();
    },
  },
};
